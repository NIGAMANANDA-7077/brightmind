            ...
            >
              {categories.map((category) => {
                ...
                return (
                  <div
                    key={category.id}
                    className={`min-w-[280px] md:min-w-[320px] p-8 rounded-[2rem] ${category.color} flex flex-col justify-between h-[320px] snap-center hover:scale-[1.02] transition-transform duration-300 cursor-pointer`}
                  >
                    <div>
                      {/* Icon Box */}
                      <div className={`w-14 h-14 ${category.iconBg || 'bg-white/50'} rounded-xl flex items-center justify-center mb-8`}>
                        <Icon className={`w-7 h-7 ${category.iconColor || 'text-gray-900'}`} />
                      </div>
